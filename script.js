...
function hasSavedCaseProgress() {
  try {
    const savedClues = JSON.parse(storage.getItem('foundClues') || '[]');
    if (Array.isArray(savedClues) && savedClues.length) return true;
  } catch (error) {
    console.warn('Unable to inspect saved clues', error);
  }

  try {
    const savedSuspects = JSON.parse(storage.getItem('interrogatedSuspects') || '[]');
    if (Array.isArray(savedSuspects) && savedSuspects.length) return true;
  } catch (error) {
    console.warn('Unable to inspect saved suspects', error);
  }

  const legacyClueCount = parseInt(storage.getItem('cluesFound') || '0', 10) || 0;
  const legacySuspectCount = parseInt(storage.getItem('suspectsInterrogated') || '0', 10) || 0;
  return legacyClueCount > 0 || legacySuspectCount > 0;
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  dom = {
    ...
    introLoadBtn:    document.querySelector('#intro-load-btn'),
  };
...
function showContinuePrompt() {
  const { continuePrompt, introLoadBtn } = dom;
  if (continuePrompt) gsap.to(continuePrompt, { opacity: 1, duration: 1.0, ease: 'power2.out' });

  if (!introLoadBtn) return;
  const hasSave = hasSavedCaseProgress();
  if (!hasSave) {
    introLoadBtn.style.display = 'none';
    introLoadBtn.style.opacity = '0';
    return;
  }

  introLoadBtn.style.display = 'block';
  gsap.to(introLoadBtn, { opacity: 1, duration: 0.9, delay: 0.18, ease: 'power2.out' });
}

function setupContinuePrompt() {
  const { continuePrompt, introLoadBtn } = dom;
  if (!continuePrompt) return;
  continuePrompt.addEventListener('click', () => beginStory({ loadSaved: false }));

  if (introLoadBtn) {
    introLoadBtn.addEventListener('click', (event) => {
      event.stopPropagation();
      beginStory({ loadSaved: true });
    });
  }
}
...
function beginStory({ loadSaved = false } = {}) {
  ...
  const { skipBtn, introScene, scrollWrapper, chapterNav, scrollHint, introLoadBtn } = dom;
  if (skipBtn) skipBtn.style.display = 'none';
  if (introLoadBtn) introLoadBtn.style.display = 'none';
...
function initChapter5_CaseFile() {
  ...
  syncCaseProgressUI();
  if (status) {
    status.textContent = hasSavedCaseProgress()
      ? 'SAVED DATA READY — USE LOAD TO RESTORE'
      : 'NO SAVED DATA — HIT SAVE FIRST';
  }
